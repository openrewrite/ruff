...
impl<'db> Provenance<'db> {
    pub fn from_definition(definition: Option<Definition<'db>>) -> Self {
...
impl<'db> Place<'db> {
    ...
    #[must_use]
    pub fn with_definition(self, definition: Definition<'db>) -> Self {
        match self {
    ...
    pub fn is_equal_ignoring_provenance(self, other: Self) -> bool {
        match (self, other) {
    ...
    pub fn raw_type(&self) -> Option<Type<'db>> {
        match self {
            Place::Defined(defined) => Some(defined.ty),
            Place::Undefined => None,
        }
...
/// ```
pub fn implicit_builtins_symbol<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    symbol: &str,
) -> PlaceAndQualifiers<'db> {
...
pub fn implicit_builtins_symbol_scope<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    symbol: &str,
) -> Option<ScopeId<'db>> {
...
#[inline]
pub fn typing_extensions_symbol<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    symbol: &str,
) -> PlaceAndQualifiers<'db> {
...
pub fn builtins_module_scope<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
) -> Option<ScopeId<'db>> {
...
pub fn place_from_bindings_with_reachability_cache<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    bindings_with_constraints: BindingWithConstraintsIterator<'_, 'db>,
...
pub fn place_from_declarations_with_reachability_cache<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    declarations: DeclarationsIterator<'_, 'db>,
    reachability_cache: &ReachabilityEvaluationCache<'db>,
...
impl<'db> PlaceFromDeclarationsResult<'db> {
    ...
    #[must_use]
    pub fn with_imported_final_for_assignment(
        self,
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
        candidates: ImportedFinalCandidatesIterator<'_, 'db>,
...
pub struct LoopHeaderReachability<'db> {
    ...
    pub deleted_narrowing_constraints: Box<[ScopedNarrowingConstraint]>,
...
pub mod implicit_globals {
    ...
    pub fn all_implicit_module_globals<'db>(
        db: &'db dyn Db,
        file: ProgramFile<'db>,
...
pub fn class_body_implicit_symbol<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    name: &str,
) -> PlaceAndQualifiers<'db> {
